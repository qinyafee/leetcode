...
// version1
// 10 = 2*5，转化成求 n! 中质因子2的个数m和质因子5 的个n数的较小值。
// 质因子5的个数 [不会大于] 质因子2的个数
// 而 n! 中质因子 5 的个数等于 [1,n]的每个数的质因子5的个数之和
...
class Solution {
 public:
  int trailingZeroes(int n) {
    int ans = 0;
    for (int i = 5; i <= n; i += 5) {
      for (int x = i; x % 5 == 0; x /= 5) {  //求x中质因子5的个数
        ++ans;
      }
    }
    return ans;
