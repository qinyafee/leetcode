...
// method1
// (1)先排序。
// (2)使用四个指针(a<b<c<d)。固定最小的a和b在左边，c=b+1,d=N-1 移动两个指针包夹求解。
// (3)保存使得nums[a]+nums[b]+nums[c]+nums[d]==target的解。偏大时d左移，偏小时c右移。
// (4)c和d相遇时，表示以当前的a和b为最小值的解已经全部求得。
// (5)b++,进入下一轮循环b循环。
// (6)当b循环结束后，a++，进入下一轮a循环。即(a在最外层循环，里面嵌套b循环，再嵌套双指针c,d包夹求解)。
// (7)去除重复解：
// 移动指针时可能出现重复数字的情况。所以我们要确保移动指针后，对应的数字要发生改变才行。
...
// method2 可以用一个hashmap先缓存两个数的和， 最终复杂度O(n )。
// ....
