// 「递归」来实现 DFS
// 本质上是对一颗节点数量为 n，形态类似字典树的多阶树进行遍历，根节点为 0，需要被跳过，因此我们可以从树的第二层开始搜索。
// https://leetcode.cn/problems/lexicographical-numbers/solution/by-ac_oier-ktn7/
 ...
 public:
  vector<int> lexicalOrder03(int n) {
    ...
    while (result.size() < n) {
      ...
      // Now, the `curr` is 200, and we need start from 2 instead of 20, that's why need keep
      // dividing 10
