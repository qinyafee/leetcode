...
//my impl
// 开始思路，难实现，否决：
// f(i,j) 表示区间 [i,j] 之间最小的cut数
// f(i, j) = min{f(i, k) + f(k + 1, j)} , i ≤ k ≤ j, 0 ≤ i ≤ j < n
// 采取思路：
// f(i)表示区间[0, i]之间最小的cut数
// 当s[j+1][i]是回文时，f(i) = min{f(i), f(j) + 1} , i ≤ j < n
...
class Solution {
 public:
  ...
  int minCut(const string& s) {
    ...
    bool p[n][n];  // is palindrome?
    fill_n(&p[0][0], n * n, false);
    for (int j = 0; j < n; ++j) {
      p[j][j] = true;
      for (int i = 0; i < j; ++i) {  //注意边界
    ...
    for (int i = 0; i < n; ++i) {
      if (p[0][i]) {  //满足回文，不需要分割
        ...
        for (int j = 0; j < i; ++j) {
          // 此行错误
          // f[i] = i;  // 最大cut， the worst case is cutting by each char
          if (p[j + 1][i]) {
            f[i] = min(f[i], f[j] + 1);
          }
        }
 ...
 *
 *  The above algorithm works well for the smaller test cases, however for the big cases, it still
 * cannot pass. Why? The way we test the palindrome is time-consuming.
 *
 *  Also using the similar DP idea, we can construct the look-up table before the main part above,
 *  so that the palindrome testing becomes the looking up operation. The way we construct the table
 * is also the idea of DP.
 *
 *  e.g.   mp[i][j]=true if str[i:j] is palindrome.
 *         mp[i][i]=true;
 *         mp[i][j] = true if str[i]==str[j] and (mp[i+1][j-1]==true or j-i<2 )  j-i<2 ensures the
 * array boundary.
 */
...
int minCutHelper(string& s, int steps, int& minSteps) {
  ...
  if (cache.find(s) != cache.end()) {
    if (s.size() > 0) cout << s << ":" << cache[s] << endl;
    return cache[s];
  }
  if (s.size() == 0) {
    if (minSteps > steps) {
