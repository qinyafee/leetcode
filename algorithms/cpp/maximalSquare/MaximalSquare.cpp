// 动态规划
// 用 dp(i,j)表示以 (i,j)为右下角，且只包含1的正方形的边长最大值。
// 如果A(i,j) = 0, 则dp(i,j) = 0
// 如果A(i,j) = 0, 则dp(i,j) = min(dp(i-1,j),dp(i,j-1), dp(i-1,j-1)) + 1
// 还需要考虑边界条件。如果 i 和 j 中至少有一个为 0，则 dp(i,j)只能是1或0
 ...
 *
 *
 *      Well, this problem desires for the use of dynamic programming. They key to any DP problem is
 * to come up with the state equation. In this problem, we define the state to be the maximal size
 * of the square that can be achieved at point (i, j), denoted as P[i][j]. Remember that we use size
 ...
 *      can be a square of size 1 while the two middle points cannot make any square, giving a size
 * of 0. Thus, P = [1, 0, 0, 1], which is the same as matrix. The case is similar for the leftmost
 * column. Till now, the boundary conditions of this DP problem are solved.
 *
 ...
 *      neighbor P[0][0] = 0, so P[1][1] can only be 1, which means that it contains the square of
 * itself.
 *
