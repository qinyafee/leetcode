...
// 动态规划
// 定义 f(i)为组成金额 i 所需最少的硬币数量, 状态转移：
// f[i] = min(f[i-cj]) + 1, cj=0,1,coins.size()-1
// https://leetcode.cn/problems/coin-change/solution/322-ling-qian-dui-huan-by-leetcode-solution/
// time:O(sn),其中 s 是总金额，n 是coin数; space:O(s)
class Solution {
 public:
  int coinChange(vector<int>& coins, int amount) {
    ...
    for (int i = 1; i <= amount; ++i) {
      ...
      for (int j = 0; j < coins.size(); ++j) {
        if (coins[j] <= i && dp[i - coins[j]] < local_min) {
          local_min = dp[i - coins[j]] + 1;
        }
        dp[i] = local_min;
      }
    }
...
class Solution {
 public:
  int coinChange(vector<int>& coins, int amount) {
    int sol[amount + 1];
    sol[0] = 0;
    for (int i = 1; i <= amount; i++) sol[i] = amount + 1;
    for (int i = 0; i < coins.size(); i++) {
      for (int j = coins[i]; j <= amount; j++) sol[j] = min(sol[j], sol[j - coins[i]] + 1);
    }
