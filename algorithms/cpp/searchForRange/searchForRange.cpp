...
class Solution {
 public:
  vector<int> searchRange(vector<int>& nums, int target) {
    // 用stl
    auto its = lower_bound(nums.begin(), nums.end(), target);
    auto ite = upper_bound(nums.begin(), nums.end(), target);
    if (its == nums.end() || *its != target) {  //注意后面的条件
      return {-1, -1};
    } else {
      int s = std::distance(nums.begin(), its);
      int e = std::distance(nums.begin(), std::prev(ite));  // std::prev，注意空vector的情况
      return {s, e};
    }
  }
  /* 自己实现的
